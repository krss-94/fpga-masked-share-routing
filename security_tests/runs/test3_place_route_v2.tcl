...
set OUTDIR      "./test3_place_route_v2_out"
set SYNTH_CKPT  "../test2_synth_out/post_synth.dcp"
set XDC_FILE    "./share_separation_v2.xdc"
...
set watch_cells {a_sh0_r a_sh1_r b_sh0_r b_sh1_r t0_sh0_reg t0_sh1_reg q_sh0_reg q_sh1_reg u_and_cross01 u_and_cross10}
...
foreach site [array names site_members] {
    ...
    if {[llength $members] > 1} {
        ...
        set has1 0
        foreach m $members {
            if {[string match "*_sh0*" $m] || [string match "*cross10*" $m]} { set has0 1 }
            if {[string match "*_sh1*" $m] || [string match "*cross01*" $m]} { set has1 1 }
        }
        if {$has0 && $has1} {
            puts "WARNING: SITE $site mixes share0-tagged and share1-tagged cells: $members"
        } else {
            puts "NOTE: SITE $site hosts multiple same-side cells: $members"
        }
    }
}
close $fh

puts "TEST 3 (v2 constraint) place/route complete. Outputs in $OUTDIR"
