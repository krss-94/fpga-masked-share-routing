"""
Milestone 8: is 155.5 degrees the best available, or just the best the
M7 sampling stride happened to find?

M7 RESULT (confirmed): open_fabric_count = 1, down from M6's 2.
INT_X27Y179 was fully eliminated; INT_X27Y178 survived, but moved one
column to INT_X26Y178 -- a real, non-trivial shift, not a relabeling.
cross_site_a's conflict (INT_X5Y8, co_located_terminal) was byte-
identical to M6, confirming the a_sh0/b_sh1 control was untouched as
intended.

M7's search wasn't exhaustive -- it strided each candidate pool down to
~30 samples (candidates[::len//30]) before checking all pairs, purely to
keep the pair-search small. That means the 155.5-degree result is the
best angle M7 happened to SAMPLE, not necessarily the best angle
AVAILABLE in the full pool.

THIS SCRIPT'S CHANGE, AND ONLY THIS: remove the stride-based sampling
and brute-force every candidate in both pools. Pool sizes here are a
few hundred sites each (n//12 of ~5220 SLICEL sites), so a full O(pool
x pool) angle-diff search is on the order of 10^5 comparisons -- trivial
for plain Python once tile column/row are cached up front (the earlier
sampled version was refetching site.getTile() through JPype inside the
inner loop, which is the real cost to avoid, not the comparison count).

Nothing else changes: same regional pools, same a_sh0/b_sh1 (untouched,
still the M6 control), same cross_site_a/b placement logic.

EXPECTED OUTCOMES, stated honestly before running:
    - If the achievable max angle is meaningfully higher than 155.5 (say,
      170+) and open_fabric drops to 0: M7's result was sampling-limited,
      not geometry-limited -- the full fix was available all along, this
      script just found it.
    - If the achievable max angle is barely above 155.5 (the die's real
      SLICEL geometry near cross_site_b just doesn't offer much closer to
      180 within these regional pools) and open_fabric stays at 1: M7's
      result was already close to the ceiling for this approach, and 1
      remaining conflict is the practical floor for angle-based
      mitigation alone -- a legitimate stopping point, not an
      under-searched one.
    - If the angle improves but open_fabric stays at 1 anyway: angle
      separation has diminishing returns past some point, and the
      remaining conflict is governed by something the angle metric
      doesn't fully capture (e.g. which specific INT tiles are adjacent
      to cross_site_b's switchbox, not just direction).

--- Milestone 7 docstring below (still accurate for the search TARGET,
    only the search METHOD changed in this script) ---
...
def pick_driver_sites(sites_sorted, used_sites, cross_site_b):
    ...
    # Milestone 8: exhaustive search, not a strided sample. Cache each
    # candidate's angle ONCE up front -- the cost that mattered in M7
    # wasn't the number of pair comparisons, it was calling
    # site.getTile() (a JPype/Java round-trip) inside the inner loop.
    # With angles cached as plain Python floats, a full pool x pool
    # search is cheap even at a few hundred candidates per side.
    scored_sh0 = [(s, angle_to_cross_b(s)) for s in candidates_sh0]
    scored_sh1 = [(s, angle_to_cross_b(s)) for s in candidates_sh1]

    print(f"  [Milestone8] exhaustive search: {len(scored_sh0)} x "
          f"{len(scored_sh1)} = {len(scored_sh0) * len(scored_sh1)} "
          f"candidate pairs (M7 sampled ~30 x 30 = 900)")

    best_pair = None
    best_angle_diff = -1
    for cand_b_sh0, ang0 in scored_sh0:
        for cand_a_sh1, ang1 in scored_sh1:
    ...
    for name, site in driver_sites.items():
        print(f"  [Milestone8] {name} -> {site.getName()} "
              f"(col={site.getTile().getColumn()}, row={site.getTile().getRow()})")
    print(f"  [Milestone8] b_sh0/a_sh1 approach-angle separation into "
          f"cross_site_b: {math.degrees(best_angle_diff):.1f} degrees "
          f"(180 = perfectly opposite directions; M7's sampled search "
          f"found 155.5)")
...
def wire_gadget(part_name, out_path):
    ...
    design = Design("masked_and_gadget_xczu2_v5", part_name)
    ...
    print("against the full series: baseline (3), M3 (6), M3b (7), M4 (3,")
    print("same tiles as baseline), M5 (4), M6 (2, driver bug fixed,")
    print("cross_site_a clean), M7 (1, angle-separated at 155.5 degrees,")
    print("155.5-degree search eliminated INT_X27Y179 and shifted")
    print("INT_X27Y178 -> INT_X26Y178). THIS run (M8) only changes HOW")
    print("thoroughly the same search space from M7 is searched -- full")
    print("brute force instead of a 30x30 stride sample. Compare the")
    print("[Milestone8] achieved angle against M7's 155.5:")
    print("  angle >> 155.5 AND open_fabric -> 0: M7 was sampling-limited,")
    print("    the full fix was reachable all along")
    print("  angle ~= 155.5 (little improvement) -> M7 was already near")
    print("    the geometric ceiling for this pool; 1 conflict is the")
    print("    practical floor for angle-based mitigation here")
    print("  angle improves but open_fabric stays at 1 -> diminishing")
    print("    returns; the residual conflict isn't fully explained by")
    print("    approach angle alone")
...
def main():
    ...
    parser.add_argument("--out", default="and_gadget_xczu2_v5_wired.dcp")
